...
class TemplateLoader
{
	public function init($template)
	{
		...
		if ( $default_file ) {
			...
			if ( ! $template ) {
				$template = WP_RENTSYST_PLUGIN_DIR . '/public/templates/' . $default_file;
			}
	...
	private static function get_template_loader_files( $default_file ) {
		...
		if ( is_tax( get_object_taxonomies( Rentsyst_CatalogVehicles::POST_TYPE_ID ) ) ) {
			...
			$templates[] = WP_RENTSYST_PLUGIN_DIR . '/public/templates/' . 'taxonomy-' . $object->taxonomy . '-' . $object->slug . '.php';
			$templates[] = 'taxonomy-' . $object->taxonomy . '.php';
			$templates[] = WP_RENTSYST_PLUGIN_DIR . '/public/templates/' . 'taxonomy-' . $object->taxonomy . '.php';
		}

		$templates[] = $default_file;
		$templates[] = WP_RENTSYST_PLUGIN_DIR . '/public/templates/' . $default_file;
